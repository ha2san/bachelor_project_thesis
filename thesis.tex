...
\chapter{Background}
%%%%%%%%%%%%%%%%%%%%
In this section, we will give a some backround  in order to understand
...
Even if the a compiled program run faster than an interpreted one, one might
...
or branches are broken, but this technique introduce too much overhead as we
need to jump from and to the trampoline at each instrumentation. \nb{Why do we
    need to do that. Eplain the challenge of
...
\chapter{Evaluation}
%%%%%%%%%%%%%%%%%%%
\nb{speaks about issue raised on github}
