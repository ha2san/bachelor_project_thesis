...
\begin{abstract}
    ...
    programs written in this language are safe\nb{what does it mean safe? -> no
    bug or at least detect them}. So \nb{"So" doesn't bring anything}our task is to test \sysname on
    ...
    information about the compilation errors\nb{why}. Also, only a few programs encountered
    ...
    We concluded that there is still \nb{possible improvements}works to do to have a fully \textit{C++} support.
...
\nb{Using only free software ...}It is unfortunately not easy to use only free software or even software whose
source code is available\nb{why?}. This can be a security problem as it is difficult
for the community to discover new bugs. Indeed, techniques such as fuzzing or
memory sanitization can only be applied by \nb{applying instrumentation at
compilation time.}modifying the binary file through the
compiler. Fortunately, a \nb{named it here, don't leave us hanging }technique makes it possible to insert instrumentation
without having access to the source code.

Binary rewriting is \nb{is used to insert}a technique used in order to insert instrumentation into
...
feel a bit too much like background. Intorduce what you did moslty! if you have
time I guess}
...
as a robust tool. \nb{this last sentence should be like 3 paragraphs}

%%%%%%%%%%%%%%%%%%%%
\chapter{Background}
%%%%%%%%%%%%%%%%%%%%
We will explain several aspects in order to allow the reader to be familiar
with any binary rewriter and more precisely \sysname. Our experience \nb{I
ddin't get what you wanting to say here}is not
...
independent in most of the cases. \nb{Never start a sentence with and}And to start executing the program takes less
...
where to find important information during the analysis \nb{which analysis}. \sysname focuses on
ELF format files and uses \textit{pyelftools}\nb{, a widely avalaible ELF parser}.
...
compiler to add instrumentation. \nb{if oyu have time, explain a bit what this
instrumenaiton is}
...
pointer of a certain type is converted into antoher\nb{typo}. Typecast are
...
\section{Challenges}
When Retrowrite was presented to the public, C++ was not yet fully supported.s
\nb{Never start wiht a conjonction like that}
...
first intention was to integrate Hextype into retrowrite. \nb{again}But then it turned
out that the task was more difficult than expected. Since not having the source
code, it was complicated to obtain the information on the classes\nb{be more precise}. It would be
...
today. \nb{a bit more precision but it looks good}
...
runtime. This is a nice result as \sysname claims that the code produced is
sound. \nb{sound == never crash}
